...
function compileSchema() {
  ...
  console.log(
    `[Prisma Switch] Target Database Provider: ${provider.toUpperCase()}`,
  );

  const schemaDir = path.join(process.cwd(), "prisma", "schema");
  if (!fs.existsSync(schemaDir)) {
    console.error(
      `[Prisma Switch] Error: Schema directory not found at ${schemaDir}`,
    );
    process.exit(1);
  }
  ...
  if (baseIndex > -1) {
    ...
    console.warn(
      `[Prisma Switch] Warning: _base.prisma not found in ${schemaDir}`,
    );
  }
  ...
  console.log(
    `[Prisma Switch] Successfully compiled and generated ${outputPath}\n`,
  );
}
