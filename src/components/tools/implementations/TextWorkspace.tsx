...
export function TextWorkspace({ defaultMode = "count" }: TextWorkspaceProps) {
  ...
  const stats = useMemo(() => {
    ...
    // Time calculations (200 wpm reading, 130 wpm speaking)
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `text_output_comparlify.txt`;
  ...
  const removeEmptyLines = () => {
    ...
    setText(lines.join("\n"));
    toast({ title: "Empty Lines Stripped", description: "All blank and whitespace-only lines removed." });
  };
  ...
  const generatedSlug = useMemo(() => {
    ...
    // Strip common stop words if enabled
    ...
    str = str
      ...
      .trim()
      .replace(/\s+/g, slugSeparator) // replace spaces
      .replace(new RegExp(`\\${slugSeparator}+`, "g"), slugSeparator); // dedupe separators
      ...
      <div className="flex items-center justify-between gap-3 bg-card/30 backdrop-blur-xl px-4 py-2.5 rounded-xl border border-border/20">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="text-xs font-semibold text-foreground truncate">
            <span className="font-bold">Zero-Server Text Editor</span> · 100% in-browser private computation.
          </p>
        </div>
      ...
      <div className="bg-card/30 backdrop-blur-xl border border-border/30 rounded-xl p-4 space-y-4">
        ...
        {activeTab === "diff" ? (
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full h-56 p-3 bg-muted/40 border border-border/30 rounded-xl text-xs font-mono outline-none resize-none"
            />
          </div>
        )}
        ...
        {activeTab === "case" && (
          <div className="space-y-3 pt-2 border-t border-border/10 animate-in fade-in duration-300">
            ...
            {/* Developer Preset Chain */}
            <div className="pt-2">
              <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest block mb-1.5">Developer Presets</span>
              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={() => applyCasing("camel")}
                  className="px-2.5 py-1 rounded-lg bg-secondary/30 text-[10px] font-bold hover:bg-secondary/50 text-foreground"
                >
        ...
        {/* TAB 4: TEXT DIFF CHECKER VIEW */}
        {activeTab === "diff" && (
          <div className="space-y-3 pt-2 border-t border-border/10 animate-in fade-in duration-300">
            <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block">Unified Visual Diff</label>
            <div className="p-3 bg-slate-950 rounded-xl border border-border/30 font-mono text-[11px] max-h-64 overflow-y-auto space-y-1">
              {diffResult.length === 0 ? (
        ...
        {activeTab === "slug" && (
          <div className="space-y-4 pt-2 border-t border-border/10 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div>
                <label className="text-[9px] font-bold uppercase text-muted-foreground block mb-1">Separator</label>
                <input
                  type="text"
                  value={slugSeparator}
                  onChange={e => setSlugSeparator(e.target.value)}
                  className="w-full h-8 px-2 bg-muted/40 border border-border/30 rounded-lg font-mono text-xs text-center"
                />
              </div>
              <div>
                <label className="text-[9px] font-bold uppercase text-muted-foreground block mb-1">Max Length</label>
                <input
                  type="number"
                  value={slugMaxLength}
              ...
              <div className="col-span-2 flex items-center gap-3 pt-4">
                <label className="flex items-center gap-1.5 text-muted-foreground cursor-pointer">
                  <input type="checkbox" checked={slugRemoveStopWords} onChange={e => setSlugRemoveStopWords(e.target.checked)} className="rounded accent-primary" /> Remove Stop Words
                </label>
              </div>
            </div>
          </div>
