...
import { generateCoursePrerequisitesAction } from '@/app/actions/ai';
...
export function CoursePrerequisitesForm() {
  const initialState = { prerequisites: null, error: null };
  const [state, formAction, isSubmitting] = useActionState(generateCoursePrerequisitesAction, initialState);
  const [courseTopic, setCourseTopic] = useState('');
  const { toast } = useToast();

  const handleCopy = () => {
    if (state.prerequisites) {
      navigator.clipboard.writeText(state.prerequisites);
      toast({
        title: 'Copied!',
        description: 'Prerequisites copied to clipboard.',
      });
    }
  };
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <Card className="shadow-lg h-full flex flex-col">
          <form action={formAction}>
            <CardHeader>
              ...
              <CardDescription>
                Enter your course topic, and the AI will generate a list of recommended prerequisites for your students.
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid w-full items-center gap-4">
                <div className="flex flex-col space-y-1.5">
                  ...
                  <Textarea
                    ...
                    placeholder="e.g., 'A beginner's guide to Python for data science.'"
                    rows={4}
                    required
                  />
        ...
        <div className="h-full">
          {state.prerequisites && !isSubmitting ? (
            <Card className="h-full flex flex-col">
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  <Sparkles className="h-6 w-6 text-primary" />
                  <CardTitle className="font-bold">Generated Prerequisites</CardTitle>
                </div>
              ...
              <CardContent className="flex-1">
                <MarkdownContent content={state.prerequisites} />
              </CardContent>
            </Card>
          ) : (
