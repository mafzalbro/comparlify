...
export function InlineROICalculator({
  ...
  const tierA = platformA.tiers.find(t => (t as any).isPopular) || platformA.tiers[0];
  const tierB = platformB.tiers.find(t => (t as any).isPopular) || platformB.tiers[0];
    ...
    <Card className="p-8 bg-card/40 backdrop-blur-3xl border border-border/10 rounded-[2.5rem] shadow-xl overflow-hidden relative group">
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            >
              <TabsList className="bg-background/50 border border-border/10 p-0.5">
                ...
                <TabsTrigger value="annually" className="text-[8px] font-black uppercase">Annual</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
          ...
          <div className="space-y-6">
            <div className="p-6 rounded-3xl bg-background/50 border border-border/10">
              <div className="flex justify-between items-end mb-4">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                  <DollarSign className="h-3 w-3 text-primary" /> Monthly Revenue
                </label>
                <span className="text-2xl font-black italic tracking-tighter">${revenue.toLocaleString()}</span>
              </div>
              <Slider
                value={[revenue]}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-background/30 border border-border/10 rounded-2xl p-4">
                <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block mb-2">Gateway</label>
                <select
                  value={gatewayId}
              ...
              <div className="bg-background/30 border border-border/10 rounded-2xl p-4">
                <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground block mb-2">Order Value</label>
                <div className="text-xs font-bold">${salesCount > 0 ? (revenue / salesCount).toFixed(0) : 0}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-center space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className={`p-5 rounded-3xl border transition-all ${savings > 0 ? "bg-muted/30 border-border/10 opacity-70" : "bg-primary/5 border-primary/20 ring-1 ring-primary/10 shadow-lg shadow-primary/5"}`}>
              <p className="text-[9px] font-black uppercase tracking-widest opacity-60 mb-1">{platformA.name}</p>
              <p className="text-2xl font-black tracking-tighter">${statsA.monthlyTotalCost.toFixed(0)}<span className="text-xs opacity-50">/mo</span></p>
              <p className="text-[8px] font-bold uppercase tracking-tight text-muted-foreground mt-1 line-clamp-1">{tierA?.name} Tier</p>
            </div>
            
            <div className={`p-5 rounded-3xl border transition-all ${savings < 0 ? "bg-muted/30 border-border/10 opacity-70" : "bg-primary/5 border-primary/20 ring-1 ring-primary/10 shadow-lg shadow-primary/5"}`}>
              <p className="text-[9px] font-black uppercase tracking-widest opacity-60 mb-1">{platformB.name}</p>
              <p className="text-2xl font-black tracking-tighter">${statsB.monthlyTotalCost.toFixed(0)}<span className="text-xs opacity-50">/mo</span></p>
              <p className="text-[8px] font-bold uppercase tracking-tight text-muted-foreground mt-1 line-clamp-1">{tierB?.name} Tier</p>
            </div>
          </div>
          ...
          {absSavings > 0 ? (
            ...
            >
              ...
              <div className="relative z-10 flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mb-2">Efficiency Savings</p>
                  <h4 className="text-3xl font-black italic tracking-tighter leading-none">
                    Save <span className="text-emerald-500 font-black">${yearlySavings.toLocaleString()}</span> <span className="text-sm font-normal text-muted-foreground">/ year</span>
                  </h4>
                  <p className="text-[10px] font-bold mt-2 text-muted-foreground">by choosing <span className="text-emerald-500 uppercase">{winner.name}</span> over {loser.name}</p>
                </div>
                <div className="h-12 w-12 rounded-2xl bg-emerald-500 flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 group-hover/result:scale-110 transition-transform">
            ...
            <div className="p-8 rounded-[2.5rem] bg-muted/30 border border-border/10 flex items-center gap-6">
              ...
              <p className="text-xs font-bold leading-relaxed text-muted-foreground italic">
                Infrastructure parity detected. Both systems present identical economic footprints at this revenue scale.
              </p>
            </div>
          )}
        </div>
