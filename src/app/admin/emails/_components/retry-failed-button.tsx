...
export function RetryFailedButton({ campaignId }: { campaignId: string }) {
  const [state, formAction] = useActionState(retryFailedEmailsAction, {
    error: null,
    success: false,
  });
  ...
  useEffect(() => {
    if (state.success) {
      toast({
        ...
        description:
          "The system is attempting to resend all failed emails for this campaign.",
      });
      router.refresh();
    }
    if (state.error) {
      ...
      <AlertDialogContent>
        <form action={formAction}>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will attempt to resend the email to all recipients who
              previously failed. Are you sure?
              {state?.error && (
