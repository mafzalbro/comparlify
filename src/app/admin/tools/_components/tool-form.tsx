...
import { availableIcons } from "@/app/(main)/tools/ai/tools";
    ...
    <form action={action}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure the details and prompt for the AI tool.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-2">
            ...
            {typeof state.error !== "string" && state?.error?.title && (
              <p className="text-destructive text-sm">{state.error.title[0]}</p>
            )}
          </div>
          <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Input
                id="inputTopicLabel"
                name="inputTopicLabel"
                defaultValue={tool?.inputTopicLabel ?? ""}
            ...
            <div className="space-y-2">
              ...
              <Input
                id="inputContextLabel"
                name="inputContextLabel"
                defaultValue={tool?.inputContextLabel ?? ""}
                placeholder="e.g., Existing Outline (optional)"
              />
            </div>
          </div>
          <div className="space-y-2">
            ...
            <Textarea
              ...
              rows={8}
              required
              placeholder="You are an expert... Use {{{topic}}} and {{{context}}} for inputs."
            />
            <p className="text-xs text-muted-foreground">
              Use Handlebars syntax. Use {"`{{{topic}}}`"} for the main input
              and {"`{{{context}}}`"} for the optional context field.
            </p>
            {typeof state.error !== "string" && state?.error?.prompt && (
              <p className="text-destructive text-sm">
                {state.error.prompt[0]}
              </p>
            )}
          </div>
