...
export function LegalDocumentForm({ document }: LegalDocumentFormProps) {
  ...
  const formAction = isEditing
    ? updateLegalDocumentAction.bind(null, document.id)
    : createLegalDocumentAction;
            ...
            <div className="space-y-2">
              ...
              {isEditing && (
                <p className="text-xs text-muted-foreground">
                  The slug cannot be changed for existing documents to preserve
                  URL integrity.
                </p>
              )}
              {typeof state.error !== "string" && state.error?.key && (
                <p className="text-destructive text-sm">{state.error.key[0]}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Editor
                ...
                onChange={setContent}
              />
              {typeof state.error !== "string" && state.error?.value && (
                <p className="text-destructive text-sm">
                  {state.error.value[0]}
                </p>
              )}
