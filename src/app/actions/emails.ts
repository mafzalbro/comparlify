...
const emailCampaignSchema = z.object({
  ...
  content: z
    .string()
    .min(20, "Email content must be at least 20 characters long."),
...
export async function updateEmailCampaign(
  ...
  try {
    ...
    if (campaign?.status !== "PENDING") {
      return {
        error: "Cannot update a campaign that is not in pending state.",
      };
    }
...
export async function sendCampaignAction(
  ...
  const validatedFields = sendCampaignSchema.safeParse(
  ...
  try {
    ...
    if (subscribers.length === 0) {
      ...
      return {
        success: true,
        error: null,
        message: "Campaign sent (0 recipients).",
      };
    }
    ...
    return {
      success: true,
      error: null,
      message: `Campaign is now sending to ${subscribers.length} recipients.`,
    };
  } catch (e) {
    console.error(e);
...
async function processEmailCampaign(campaignId: string) {
  ...
  for (const recipient of campaign.recipients) {
    if (recipient.user.email) {
      try {
        const unsubscribeToken = Buffer.from(recipient.user.id).toString(
          "base64",
        );
  ...
  const remainingPending = await prisma.emailRecipient.count({
...
export async function retryFailedEmailsAction(
  ...
  const content = await getContent();
  const siteName = content["global.siteName"] || "Comparlify";
  const { campaignId } = sendCampaignSchema.parse(
    Object.fromEntries(formData.entries()),
  );
