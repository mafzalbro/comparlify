  ...
  try {
    ...
    if (error && typeof error === "object" && "code" in error) {
      ...
      if ((error as any).code === "P2003") {
        return {
          error:
            "Cannot delete feature. It is currently in use by one or more platforms.",
        };
      }
    }
    console.error(error);
...
export async function createFeatureCategory(
  ...
  const validatedFields = featureCategorySchema.safeParse(
    Object.fromEntries(formData.entries()),
  );
...
export async function deleteFeatureCategory(
  prevState: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const session = await auth();
