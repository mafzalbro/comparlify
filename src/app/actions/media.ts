...
export async function updateImageDetailsAction(
  input: z.infer<typeof updateImageSchema>,
) {
  const session = await auth();
...
export async function getImageUsage(imageUrl: string): Promise<ImageUsage[]> {
  ...
  for (const article of newsArticles) {
    usages.push({
      type: "News Article",
      title: article.title,
      url: `/admin/news/edit/${article.id}`,
    });
  }
  ...
  for (const platform of platforms) {
    usages.push({
      type: "Platform Logo",
      title: platform.name,
      url: `/admin/platforms/edit/${platform.id}`,
    });
  }
